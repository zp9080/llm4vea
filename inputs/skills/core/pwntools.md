...
- **【必须】泄露地址解析（禁止直接用 `data[:8]`）**
  - **正确模式**：`leaked = u64(p.recvuntil(b'\x7f')[-6:].ljust(8, b'\x00'))`
  - **原理**：libc 地址的高字节总是 `0x7f`，通过 `recvuntil(b'\x7f')` 可以精确定位泄露的 libc 地址。
  - **风险**：其他接收方式（如固定长度 `data[:8]`）容易受到输出格式变化的影响，导致地址解析错误，最终使 exp 失败。
